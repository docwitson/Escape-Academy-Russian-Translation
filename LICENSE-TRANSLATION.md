# Russian translation license

The Russian translation text in `ollama_pipeline/output/` is made available
under the [Creative Commons Attribution-NonCommercial-ShareAlike 4.0
International License](https://creativecommons.org/licenses/by-nc-sa/4.0/),
to the extent that copyright or similar rights exist in that text and are held
by the project's contributors.

SPDX identifier: `CC-BY-NC-SA-4.0`

Attribution may be given as:

> Escape Academy Russian Translation — docwitson and contributors

The translation was produced primarily with the local language model
Qwen3.8-57K and subsequently checked with automated tools and in the game.

This license does not cover Escape Academy, its original text, trademarks,
assets or any other material owned by the game's developers, publishers or
other rightsholders. Nothing in this notice grants permissions that the
project's contributors do not have the authority to grant.
